...
export const getPatientMedicines = async (
  ...
  try {
    const { nin_hash } = req.params;
    console.log('📋 Fetching medicines for nin_hash:', nin_hash);

    // Get patient by nin_hash
    const { data: patient, error: patientError } = await supabase
      .from('Patients')
      .select('id')
      .eq('nin_hash', nin_hash)
    ...
    if (patientError) {
      ...
      if (patientError.code === 'PGRST116') {
        res.status(404).json({ message: 'Patient not found', medicines: [], diagnoses: [], treatments: [] });
        return;
      }
      throw patientError;
    }
    ...
    if (!patient) {
      console.warn('⚠️ Patient not found for nin_hash:', nin_hash);
      res.status(404).json({ message: 'Patient not found', medicines: [], diagnoses: [], treatments: [] });
      return;
    }
    ...
    console.log('✅ Found prescriptions:', prescriptionIds.length);
    ...
    if (prescriptionIds.length > 0) {
      ...
      if (medicinesError) {
        console.error('❌ Medicines Error:', medicinesError);
      ...
      const { data: treatmentsData, error: treatmentsError } = await supabase
        .from('Treatments')
        .select('id, treatment_type, description, instructions, created_at')
        .in('prescription_id', prescriptionIds)
        .order('created_at', { ascending: false });
    ...
    const { data: diagnosesData, error: diagnosesError } = await supabase
      .from('Diagnoses')
      .select('id, diagnosis_name, icd_code, notes, created_at')
      .eq('patient_id', patient.id)
      .order('created_at', { ascending: false });
    ...
    console.error('🚨 Error in getPatientMedicines:', error);
    next(error);
  }
};
